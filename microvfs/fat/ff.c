...
)
{
    ...
    *rfs = fs;                            /* Return pointer to the file system object */
    ...
    stat = disk_initialize(fs->bdev);    /* Initialize the physical drive */
...
)
{
    ...
    if (!fil || !fil->fs || !fil->fs->fs_type || fil->fs->id != fil->id || (disk_status(fil->fs->bdev) & STA_NOINIT))
